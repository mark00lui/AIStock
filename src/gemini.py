...
2. 對於台灣股票（如 3019.TW），請搜尋：
   ...
   - 中文譯名參考：請參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的股票中文名稱
3. 對於美股，請搜尋：
   - 英文：公司英文名稱、股票代碼、美國市場新聞、華爾街分析
   - 中文：公司中文譯名、相關中文報導、中國財經媒體
   - 中文譯名參考：請參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的美股中文譯名
4. 對於港股，請搜尋：
   - 中文：公司中文名稱、香港市場新聞、港媒報導
   - 英文：公司英文名稱、國際市場相關新聞、外資分析
   - 中文譯名參考：請參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的港股中文名稱
5. 搜尋範圍包括：財經新聞、分析師報告、社群媒體討論、財報公告、監管文件
6. 重要：所有股票的中文譯名都必須參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的官方譯名，確保準確性和一致性
...
4. 確保搜尋到最新的中英文新聞資料
5. 只返回JSON格式，不要其他說明
6. 股票中文譯名必須參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的官方譯名，確保準確性
7. 風險指標必須提供精確的數值：
...
搜尋策略要求：
1. 同時搜尋英文和中文資料源，確保全面性
2. 對於台灣股票（如 XXXX.TW or XXXX.TWO），請搜尋：
   - 中文：公司中文名稱、股票代碼、相關產業新聞、台灣財經媒體
   - 英文：公司英文名稱、股票代碼、國際市場相關新聞、外資報告
   - 中文譯名參考：請參考 台灣股票交易所 的股票中文名稱
3. 對於美股，請搜尋：
   - 英文：公司英文名稱、股票代碼、美國市場新聞、華爾街分析
   - 中文：公司中文譯名、相關中文報導、中國財經媒體
   - 中文譯名參考：請參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的美股中文譯名
...
4. 確保搜尋到最新的中英文新聞資料
5. 只返回JSON格式，不要其他說明
6. 股票中文譯名必須參考 台股請參考 台灣股票交易所 的官方譯名，美股及港股請參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的官方譯名，確保準確性
7. 風險指標必須提供精確的數值：
   - beta: 必須是數字（如1.25），表示相對於市場指數的Beta值
   - volatility: 必須是數字（如35.5），表示年化波動率百分比
   - sharpe_ratio: 必須是數字（如0.85），表示夏普比率
...
搜尋策略要求：
1. 同時搜尋英文和中文資料源，確保全面性
2. 對於台灣股票（XXXX.TW or XXXX.TWO），請搜尋：
   - 中文：公司中文名稱、股票代碼、相關產業新聞、台灣財經媒體
   - 英文：公司英文名稱、股票代碼、國際市場相關新聞、外資報告
   - 中文譯名參考：請參考 台灣股票交易所 的股票中文名稱
3. 對於美股，請搜尋：
...
{{
    ...
    "data_sources": {{
        "news_sources": ["路透社", "彭博社", "CNBC", "金融時報", "華爾街日報", "經濟日報", "工商時報", "聯合報", "其他來源"],
        "social_media_platforms": ["X.com", "Threads", "Reddit", "Twitter", "StockTwits", "其他平台"],
...
分析要求：
0. 請基於最近一季的分析，以及分析師對接下來的季營收的預測以及X.com / Threads 等社群上的氣紛
1. 基於最新的新聞報導、社群討論和分析師觀點，尤其是出現訴訟、專利糾紛、管理層變動、產品發布、技術創新、併購、合作和戰略投資等新聞
2. 提供具體的影響評估和市場反應預期，以及對於股價的影響，尤其是判斷它是不確定性，好消息還是壞消息，股價對於不確定性，永遠比好消息或壞消息來的敏感
3. 考慮新聞的時間敏感性和持續影響，以及對於股價的影響
4. 分析必須客觀、全面，避免過度反應，避免過度樂觀或悲觀，股價對於不確定性，永遠比好消息或壞消息來的敏感
5. 返回格式必須是有效的 JSON，所有描述必須具體
6. 所有分析結果都必須使用繁體中文表達，包括所有指標、建議和描述，避免使用簡體中文用詞，使用台灣地區的金融術語和表達方式
7. 避免使用簡體中文用詞，使用台灣地區的金融術語和表達方式
8. 股票中文譯名必須參考 Yahoo 股市台灣網站 (tw.stock.yahoo.com) 的官方譯名，確保準確性
